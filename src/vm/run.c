...
static paw_loaded_library_t *load_paw_library(const char *requested) {
    ...
    if (g_library_count >= MAX_PAW_LIBRARIES) {
        fprintf(stderr, "Paw FFI: too many loaded libraries\n");
        return NULL;
    }
    ...
    if (!resolved) {
        fprintf(stderr, "Paw FFI: library '%s' was not found\n", requested);
        return NULL;
    }
    ...
    if (!handle) {
        fprintf(stderr, "Paw FFI: failed to load '%s'\n", resolved);
        free(resolved);
        return NULL;
    }
    ...
    if (!info_symbol) {
        fprintf(stderr, "Paw FFI: '%s' does not export paw_library_info\n", resolved);
    ...
    if (!info) {
        fprintf(stderr, "Paw FFI: '%s' returned NULL library metadata\n", resolved);
        dlFreeLibrary(handle);
        free(resolved);
        return NULL;
    }
    ...
    if (info->abi_version != PAW_FFI_ABI_VERSION) {
        fprintf(stderr, "Paw FFI: ABI mismatch in '%s' (library=%u runtime=%u)\n", resolved, info->abi_version, PAW_FFI_ABI_VERSION);
        dlFreeLibrary(handle);
        free(resolved);
        return NULL;
    }
    ...
    if (!info->functions && info->function_count != 0) {
        fprintf(stderr, "Paw FFI: malformed function table in '%s'\n", resolved);
        dlFreeLibrary(handle);
        free(resolved);
        return NULL;
    }
...
static void ffi_push_argument(VM *vm, paw_ffi_type_t type, uintptr_t value) {
    switch (type) {
        ...
        case PAW_FFI_CSTRING: {
            const char *str = VM_get_string(vm, value);
...
static uintptr_t ffi_call(VM *vm, const paw_ffi_function_t *fn) {
    ...
    for (uint32_t i = 0; i < fn->arg_count; ++i) {
        if (i >= 15) {
            fprintf(stderr, "Paw FFI: too many arguments\n");
            return 0;
        }
    ...
    switch ((paw_ffi_type_t)fn->return_type) {
        ...
        case PAW_FFI_CHAR:
            return (uintptr_t)dcCallChar(g_vm_ffi, fn->address);

        case PAW_FFI_INT:
            return (uintptr_t)dcCallInt(g_vm_ffi, fn->address);

        case PAW_FFI_POINTER:
            return (uintptr_t)dcCallPointer(g_vm_ffi, fn->address);

        case PAW_FFI_CSTRING: {
            const char *result = (const char *)dcCallPointer(g_vm_ffi, fn->address);

            if (!result) return 0;

            int id = vm_register_string(result);
        ...
        default:
            fprintf(stderr, "Paw FFI: unsupported return type %u\n", fn->return_type);
            return 0;
    }
}
    ...
    switch (sys_code) {
        ...
        case 3: {
            ...
            if (!library_name || !symbol_name) {
                fprintf(stderr, "Paw FFI: invalid library/function name\n");
                vm->regs[0] = 0;
                break;
            }
            ...
            if (!function) {
                fprintf(stderr, "Paw FFI: '%s' not exported by '%s'\n", symbol_name, library_name);
                vm->regs[0] = 0;
                vm->is_running = 0;
                break;
            }
            ...
            if (function->arg_count > 15) {
                fprintf(stderr, "Paw FFI: '%s' has too many arguments\n", symbol_name);
                vm->regs[0] = 0;
                vm->is_running = 0;
                break;
            }
            ...
            if (!function_id) {
                fprintf(stderr, "Paw FFI: function table exhausted\n");
                vm->regs[0] = 0;
                vm->is_running = 0;
                break;
            }
        ...
        case 4: {
            ...
            if (!runtime_fn) {
                fprintf(stderr, "Paw FFI: invalid function ID %u\n", function_id);
                vm->regs[0] = 0;
                vm->is_running = 0;
                break;
            }

            vm->regs[0] = ffi_call(vm, runtime_fn->function);
            break;
        }
