...
void compile_node(nu_ast_node_t *node) {
    ...
    switch (node->type) {
        case AST_ROOT:
            for (nu_ast_node_t *child = node->first_child; child != NULL; child = child->next_sibling) compile_node(child);
            break;
        ...
        case AST_BLOCK:
            for (nu_ast_node_t *stmt = node->first_child; stmt != NULL; stmt = stmt->next_sibling) compile_node(stmt);
            break;
...
void walk_ast_to_file(nu_ast_node_t *node, const char *out_filename) {
    ...
    if (main_fn) {
        for (nu_ast_node_t *child = node->first_child; child != NULL; child = child->next_sibling) {
            if (child->type == AST_CONST_DECL || child->type == AST_INT_DECL || child->type == AST_CHAR_DECL) compile_node(child);
        }
